...
exports.run = async (bot, message, args) => {
  if (!args[0]) return message.channel.send('Bu Komutu Kullanabilmek için `aç` yada `kapat` yazmalısın.')
  if(!ayarlar.includes(args[0])) return message.channel.send(`Geçerli komutları kullanmalısın.\nKomutlar: ${ayarlar.join(' - ')}`)
