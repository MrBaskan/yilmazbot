...
if(!kanal) return message.channel.send(`Jail log kanalı ayarlı değil.`)
  ...
  if(!kişi) return message.channel.send(`Kimi hapise atacaksın? Etiketlemeyi unutma.`)
  if(kişi.hasPermission("MANAGE_GUILD")) return message.channel.send(`Olmaz. Bu kişiyi hapse atamam.`)
  let zaman = args[1]
  if(!args[1]) return message.channel.send(`Ne kadar süre jailde duracağını belirtmelisin.\nÖrnek: y!jail kişi süre sebep`)
let sebep = args.join(' ').slice(args[1].length+args[0].length + 1)
if(!sebep) sebep = 'Sebep belirtilmemiş.'
  const wasted = new Discord.MessageEmbed()
  .setAuthor(message.author.tag, message.author.avatarURL())
  .setColor(`PURPLE`)
  .setDescription(`Yine biri hapishaneye yollandı.`)
